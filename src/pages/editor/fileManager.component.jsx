                ...
                <div style={{marginTop: "2rem", paddingLeft: '25px'}}>
                    <Space>
                        ...
                        <Button size="small" icon={<FileAddOutlined/>} type="primary"
                                onClick={_ => this.setState({visibleNewFileModal: true})}> File </Button>

                        {/* <Button size="small"  icon={<FolderAddOutlined/>} type="primary" onClick={_=>this.setState({visibleNewFolderModal: true})} > Folder </Button> */}
                        <Popconfirm disabled={!this.context.selectedId || this.context.selectedId === 'temproom'}
                                    placement="top" title="Are you sure？"
                                    icon={<QuestionCircleOutlined style={{color: "red"}}/>}
                                    onConfirm={this.handleDelete}>
                            <Button size="small" icon={<DeleteOutlined/>} type="primary" danger
                                    disabled={!this.context.selectedId || this.context.selectedId === `${this.props.roomName}`}> Delete </Button>
                        </Popconfirm>
                        <Button size="small" icon={<CaretRightOutlined/>} type="primary"
                                disabled={!this.context.activeKey || this.context.termFileName === null}
                                onClick={this.context.runTerminal}>Run </Button>
