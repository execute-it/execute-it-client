...
export default class ParticipantsList extends React.Component {
    ...
    this.subscription = this.props.activity
      ...
      .subscribe((participants) => {
        ...
        participants.forEach((participant) => {
          ...
          if (
            !JSON.parse(participant.user.displayName).isBot &&
  ...
  createParticipant(participant, isSelf) {
    ...
    const streams = this.context.audioStreams[
      JSON.parse(participant.user.displayName).email
    ];
    return (
      <Participant
  ...
  getVoiceStatus = (status) => {
    if (status)
      return (
        <Typography.Text
          style={{ color: "#58e411", fontWeight: "bold" }}
          level={4}>
          {"Voice Connected"}
        </Typography.Text>
      );
      ...
      return (
        <Typography.Text
          style={{ color: "#e4ab0a", fontWeight: "bold" }}
          level={4}>
          {"Connecting Voice..."}
        </Typography.Text>
      );
